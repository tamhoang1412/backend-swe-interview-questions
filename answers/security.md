...
SSL - Secure Socket Layer, is the original encryption protocol developed for HTTP. It now has been replaced the TLS, Transport Layer Security, but the name SSL is still widely in used. Whenever a client makes a connection to a server over HTTPS, the client and server will initiate a TLS handshake, which happens after the TCP handshake has been finished.

The steps of a TLS handshake will vary depends on the key exchange algorithm in use. The RSA key exchange algorithm is used most often. The steps are as follow:
...
TLS handshake can also use Diffie-Hellman as an key exchange algorithm. The steps then will be:

1. Client's hello is same as above
2. Server's hello also sends the server's random, chosen cipher suite, SSL ceritificate. The server also include a digital signature, created by encrypting the client random, the server random, and its (Diffie-Hellman) DH parameter using the private key
